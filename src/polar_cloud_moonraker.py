...
class PolarCloudPlugin:
    def __init__(self, config):
        ...
        # Get Moonraker's HTTP client for async requests
        self.http_client = self.server.lookup_component('http_client', None)
    ...
    async def _http_get(self, url: str, timeout: float = 10.0) -> Optional[Any]:
        """Make an async HTTP GET request using Moonraker's HTTP client."""
        if self.http_client is None:
            logging.warning("HTTP client not available")
            return None
        try:
    ...
    async def _resolve_dns(self, hostname: str) -> Optional[str]:
        ...
        try:
            ...
            logging.debug(f"DNS resolution failed for {hostname}: {e}")
            return None
    ...
    async def _handle_register_request(self, web_request):
        ...
        try:
            ...
            # Restart service to pick up new config (async)
            if is_k1_system():
                result = await self._run_command(
                ...
                result = await self._run_command(
                    ["systemctl", "restart", "polar_cloud.service"],
                    timeout=30.0
                )

            if result['returncode'] != 0:
                logging.error(f"Error restarting polar cloud service: {result['stderr']}")
    ...
    async def _handle_config_request(self, web_request):
        ...
        try:
            if web_request.get_action() == "GET":
                ...
                # Restart service if needed (async)
                if is_k1_system():
                    result = await self._run_command(
                        ["/usr/data/polar_cloud_service.sh", "restart"],
                        timeout=30.0
    ...
    async def _handle_export_logs_request(self, web_request):
        ...
        try:
            ...
            result = await self._run_command(['uname', '-a'], timeout=5.0)
            if result['returncode'] == 0:
                logs.append(f"System: {result['stdout'].strip()}")
            else:
                logs.append("System: Unable to retrieve")

            result = await self._run_command(['uptime'], timeout=5.0)
            if result['returncode'] == 0:
            ...
            # Test DNS resolution (async)
            resolved_ip = await self._resolve_dns('printer4.polar3d.com')
            if resolved_ip:
                logs.append("✓ DNS Resolution: printer4.polar3d.com resolved successfully")
            else:
                logs.append("✗ DNS Resolution: Failed to resolve printer4.polar3d.com")

            # Test ping to Polar Cloud (async)
            result = await self._run_command(
                ['ping', '-c', '3', '-W', '5', 'printer4.polar3d.com'],
                timeout=20.0
            )
            if result['returncode'] == 0:
            ...
            # Test HTTPS connectivity (async)
            response = await self._http_get('https://printer4.polar3d.com', timeout=10.0)
            if response is not None:
                logs.append(f"✓ HTTPS Test: printer4.polar3d.com responded with status {response.status_code}")
            else:
                logs.append("✗ HTTPS Test: Failed to connect")
            logs.append("")
            ...
            if is_k1_system():
                ...
                result = await self._run_command(
                    ['systemctl', 'is-active', 'polar_cloud'],
                    timeout=5.0
                )
                ...
                if status == "active":
                    # Get service details
                    result = await self._run_command(
                        ['systemctl', 'status', 'polar_cloud', '--no-pager', '-l'],
                        timeout=10.0
                    )
            ...
            if is_k1_system() and os.path.exists(log_file):
                ...
                result = await self._run_command(
                    ['journalctl', '-u', 'polar_cloud', '-n', '200', '--no-pager'],
                    timeout=15.0
                )
                if result['returncode'] == 0:
                    logs.append(result['stdout'])
    ...
    async def _get_version_info(self):
        """Get version information from git (async)"""
        try:
            ...
            # Get current version from git tags (async)
            result = await self._run_command(
            ...
            # Check for latest version from GitHub (async)
            latest_version = None
    ...
    async def _handle_update_request(self, web_request):
        ...
        try:
            ...
            if result['returncode'] == 0:
                ...
                return {
                    "success": False,
                    "error": f"Service restart failed: {result['stderr']}"
                }
